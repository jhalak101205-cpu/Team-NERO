...
# Feature 3: Grounded KPI Data Loader & Global In-Memory Store
def load_kpi_data():
    kpi_file_path = os.path.join(BASE_DIR, "kpi_data.json")
    try:
        with open(kpi_file_path, "r", encoding="utf-8") as f:
            data = json.load(f)
            print(f"✅ Loaded KPI Data from '{kpi_file_path}'.")
            return data
    except Exception as e:
        print(f"⚠️ Error loading kpi_data.json: {e}")
        return {}
...
def startup_event():
    ...
    global embedding_model, chroma_collection, KPI_DATA
...
# Pydantic schema for POST /chat request body
...
# Feature 3: Grounded AI Chatbot Endpoint (Fixed KPI Table)
@app.post("/chat")
def chat_endpoint(request: ChatRequest):
    """
    Feature 3: Grounded AI Chatbot Endpoint.
    Answers user questions strictly using ONLY the in-memory KPI_DATA fact table.
    Does NOT use ChromaDB, embeddings, or document search.
    Calls Gemini using the exact same genai.GenerativeModel.generate_content method.
    """
    question = request.question.strip()
    if not question:
        raise HTTPException(status_code=400, detail="Question string cannot be empty.")
    ...
    # 2. Convert entire in-memory KPI_DATA table into formatted text
    global KPI_DATA
    if not KPI_DATA:
        KPI_DATA = load_kpi_data()

    kpi_text = json.dumps(KPI_DATA, indent=2)

    # 3. Build prompt with strict grounding instructions
    system_instruction = (
        "You are a strict, factual land governance assistant. "
        "Answer the user's question using ONLY the provided fixed table of statistics below. "
        "Do NOT guess, extrapolate, or use general AI knowledge. "
        "If the user asks about a state, metric, or statistic that is NOT present in the provided table, "
        "you MUST honestly respond: 'I don't have that information in the provided data.'"
    )

    prompt = (
        f"{system_instruction}\n\n"
        f"FIXED KPI DATA TABLE (JSON):\n{kpi_text}\n\n"
        f"USER QUESTION: {question}\n\n"
        f"ANSWER:"
    )

    # 4. Call Gemini using the exact same function/method as /ask
    try:
        genai.configure(api_key=gemini_api_key)
        try:
            model = genai.GenerativeModel("gemini-2.0-flash")
